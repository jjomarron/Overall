...
loan<-read.csv("http://bigblue.depaul.edu/jlee141/econdata/eco520/loan_default.csv")
str(loan)
...
#put all graphs together to compare
par(mfrow=c(2,3))
auc_plot(yhat0,testy,"LPM")
auc_plot(yhat1,testy,"LPM")
auc_plot(yhat2,testy,"LPM")
...
#save the algorithm
saveRDS(lm2,file="C:/Users/Jack/Documents/School/Business Analytics.520/loan_LPM.rds")
saveRDS(logit2,file="C:/Users/Jack/Documents/School/Business Analytics.520/loan_logit.rds")

#Random Forest model
rf1<-randomForest(formula=Default~.,data=train,mtry=5,ntree=500)
summary(rf1)
...
for(i in 1:12){
  temp.model <-randomForest(formula=Default~.,data=train,mtry=i,ntree=500)
...
#find the best ntree
rf_tree<-randomForest(formula=Default~.,data=train,mtry=3,ntree=1000)
Trees<-rep(1:nrow(rf_tree$err.rate))
Error.rate<-rf_tree$err.rate[,"OOB"]
plot(Trees,Error.rate,col="red")

#tree might be 100 or 450
rf2<-randomForest(formula=Default~.,data=train,mtry=3,ntree=100)
summary(rf2)
...
rf3<-randomForest(formula=Default~.,data=train,mtry=3,ntree=450)
summary(rf3)
...
par(mfrow=c(2,2))
auc_plot(rfhat1,testy,"RANDOMFOREST mtry=5 ntree=500")
...
saveRDS(rf1,file="C:/Users/Jack/Documents/School/Business Analytics.520/loan_randomf1.rds")
